...
class AtCmd:
    ...
    def set_test_mode_nr(self):  # SA: 0, NSA: 1
        """
        SA: 0, NSA: 1
        """
    ...
    def set_test_end_nr(self, delay=0.2):
        logger.info('----------Set End----------')
    ...
    def sync_nr(self):
        logger.info('---------Sync----------')
        if self.bw_nr not in self.bw_nr_dict.keys():
            raise DictionaryException(f'There is not {self.bw_nr} in self.bw_nr_dict')
        scs = 1 if self.band_nr in TDD_BANDS and self.band_nr not in NTN_BANDS else 0
        response = self.command(
            f'AT+NRFSYNC={self.sync_path_dict[self.sync_path]},{self.sync_mode},{scs},'
            f'{self.bw_nr_dict[self.bw_nr]},0,{self.rx_freq_nr}',
            delay=1)
        while b'+NRFSYNC:1\r\n' not in response:
    ...
    def tx_set_nr(self):
        logger.info('---------Tx Set----------')
        self.command(
            f'AT+NTXSENDREQ={self.tx_path_dict[self.tx_path]},{self.tx_freq_nr},{self.bw_nr_dict[self.bw_nr]},'
            f'{self.scs_dict[self.scs]},{self.rb_size_nr},{self.rb_start_nr},{self.mcs_nr_dict[self.mcs_nr]},'
            f'{self.type_dict[self.type_nr]},{self.tx_level}')
        logger.info(
    ...
    def tx_set_no_sync_nr(self):
        logger.info('---------Tx No Sync----------')
        self.scs = 30 if self.band_nr in TDD_BANDS else 15
        self.command(
            f'AT+NRFACTREQ={self.tx_path_dict[self.tx_path]},{self.tx_freq_nr},{self.bw_nr_dict[self.bw_nr]},'
            f'{self.scs_dict[self.scs]},{self.rb_size_nr},{self.rb_start_nr},{self.mcs_nr_dict[self.mcs_nr]},'
            f'{self.type_dict[self.type_nr]},{self.tx_level},{self.uldl_period},{self.dl_slot},{self.dl_symbol},'
            f'{self.ul_slot},{self.ul_symbol}')
        logger.info(
            f'TX_PATH: {self.tx_path}, BW: {self.bw_nr}, TX_FREQ: {self.tx_freq_nr}, RB_SIZE: {self.rb_size_nr}, '
            f'RB_OFFSET: {self.rb_start_nr}, MCS: {self.mcs_nr}, TX_LEVEL: {self.tx_level}, '
    ...
    def rx_path_setting_nr(self):
        """
        2: PRX, 1: DRX, 4: RX2, 8:RX3, 3: PRX+DRX, 12: RX2+RX3, 15: ALL PATH
        """
        if self.rx_path_nr is None:
            self.rx_path_nr = 15
        logger.info('----------Rx path setting----------')
        logger.info(f'----------Now is {self.rx_path_nr_dict[self.rx_path_nr]}---------')
        self.command(f'AT+NRXMODESET={self.rx_path_nr}')
        # self.command_cmw100_query('*OPC?')
    ...
    def query_rsrp_cinr_nr(self):
        res = self.command(f'AT+NRXMEAS={self.rx_path_nr},20')
        for line in res:
    ...
    def set_level_nr(self, tx_level):
    ...
    def query_voltage_nr_sky51001(self, band, tx_path):
        """
        P23:
        MIPI 0 = MHB/ UHB Sub
        MIPI 2 = LB / MHB ENDC
        MIPI 4 = UHB Main
        """
        mipi_num, usid, addr = self.set_mipi_voltage_sky51001('NR', band, tx_path)
    ...
    def query_voltage_nr_qm81052(self, band, tx_path):
        """
        P24:
        ...
        """
        mipi_num, usid, addr = self.set_mipi_voltage_qm81052('NR', band, tx_path)
    ...
    def get_used_band_index_by_path_nr(self, tx_path):
        """
        To transfer used_band to {band: index, ...}
        used_band_name parameter that can be valid:
        'CAL.NR_SUB6.USED_RF_BAND'
    ...
    def mpr_nvs_check(self, band):
        ...
        used_band_tx2_index_nr = self.get_used_band_index("CAL.NR_SUB6.USED_DUALTX_RF_BAND")
        # mpr_nvs = [
        ...
        #     f'!NR_SUB6RF.TX.USER MPR OFFSET TX0_N{str(used_band_tx1_index_nr[band]).zfill(2)}',
        #     f'!NR_SUB6RF.TX.USER MPR OFFSET TX1_N{str(used_band_tx2_index_nr[band]).zfill(2)}',
        #     f'!NR_SUB6RF.TX.USER MPR OFFSET PC2 TX0_N{str(used_band_tx1_index_nr[band]).zfill(2)}',
        #     f'!NR_SUB6RF.TX.USER MPR OFFSET PC2 TX1_N{str(used_band_tx2_index_nr[band]).zfill(2)}',
        #     f'!NR_SUB6RF.TX.USER MPR OFFSET PC1p5 TX0_N{str(used_band_tx1_index_nr[band]).zfill(2)}',
        #     f'!NR_SUB6RF.TX.USER MPR OFFSET PC1p5 TX1_N{str(used_band_tx2_index_nr[band]).zfill(2)}',
        #
        # ]
        ...
        for mpr_nv in mpr_nvs:
            try:
                if mpr_nv == '!LTERF.TX.USER DSP MPR OFFSET TX0 B' or mpr_nv == '!LTERF.TX.USER DSP MPR INTRA_CA OFFSET TX B':
                    ...
                    mpr_nvs_new.append(mpr_nv + f'{str(used_band_tx1_index_nr[band]).zfill(2)}')

                elif 'TX1_N' in mpr_nv:
                    mpr_nvs_new.append(mpr_nv + f'{str(used_band_tx2_index_nr[band]).zfill(2)}')
            except KeyError:
    ...
    def set_apt_internal_calibration_nr(self, tx_path, freq):
        """
    ...
    def set_calibration_finish_nr(self):
        """
        Calibration mode finished
        """
        self.command(f'AT+NRFCALFINISH', 0.5)

    def apt_calibration_process_nr(self, band, tx_path, freq):
        self.set_stop_network_cal()
        self.set_postcal_nr(band)
        self.set_apt_internal_calibration_nr(tx_path, freq)
        self.set_calibration_finish_nr()

    def set_apt_vcc_nv_nr(self, vcc_para_dict):
        """
        AT+NTXAPTVOLNVWRITE=P0,P1,~P37
        """
        band = vcc_para_dict['band']
        scheme = self.apt_vcc_dict_nr[vcc_para_dict['scheme']]
    ...
    def set_apt_bias_nv_nr(self, bias_num, bias_para_dict):
        """
        AT+NTXAPTBIASNVWRITE=P0,P1,~P38
        """
        band = bias_para_dict['band']
        scheme = self.apt_vcc_dict_nr[bias_para_dict['scheme']]
    ...
    def set_apt_vcc_nv_each_cp_nr(self, band, tx_path, pa_mode, index_wanted, value_wanted):
        """
        used for the NV:
        ...
        if tx_path == 'TX1':
            ...
            used_band_index_nr = self.get_used_band_index('CAL.NR_SUB6.USED_DUALTX_RF_BAND')

        # get the NV we want
        vcc_nv_wanted = f'CAL.NR_SUB6.TX_APT_DC_TABLE_MIDCH_{pa_mode}PM_TX{self.tx_path_dict[self.tx_path]}' \
                        f'_N{str(used_band_index_nr[band]).zfill(2)}'
    ...
    def set_apt_bias_nv_each_cp_nr(self, band, tx_path, bias_num, pa_mode, index_wanted, value_wanted):
        """
        used for the NV:
...
if __name__ == '__main__':
    ...
    #
    command = AtCmd()
    # command.apt_calibration_process_nr(1, 'TX1', 1950000)
